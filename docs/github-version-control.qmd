...
Git is a history of snapshots **on your machine**. GitHub is a server that stores a copy of that history so other people (and future you) can `clone` it. The link between the two is a **remote**. Official docs: [Git](https://git-scm.com/doc), [GitHub Docs](https://docs.github.com/en), and the curated list in [References — Version control and publishing](references.qmd#version-control-and-publishing).
...
**[Git LFS](https://git-lfs.com/)** is for files that *belong* with the source and are large-but-stable: robot meshes (`.obj`, `.usd`), a small canonical MJCF mesh pack, a fixture binary a test must have. LFS stores a **pointer** in git and the bytes on a separate LFS server.
...
## Third-party assets: sparse checkout {#third-party-assets-sparse-checkout}

When a dependency is **large, upstream-maintained, and not your source code** (robot
models, sim environments, datasets), prefer:

1. **Do not** copy meshes into your repo (even with LFS) unless you own the asset.
2. **Do** commit a **lock file** + **fetch script** that pins a git ref.
3. **Do** add the checkout directory to **`.gitignore`**.

This repo’s pattern (`third_party/assets.lock.json` + `scripts/fetch_menagerie_assets.sh`):

```bash
# Creates third_party/mujoco_menagerie/ with only agilex_piper/ and arx_l5/
bash scripts/fetch_menagerie_assets.sh
```

Under the hood: `git clone --filter=blob:none --sparse`, then
[`git sparse-checkout set`](https://git-scm.com/docs/git-sparse-checkout) `agilex_piper arx_l5`, then `git checkout <pinned-sha>`.
Everyone gets the same robots without cloning all of [MuJoCo Menagerie](https://github.com/google-deepmind/mujoco_menagerie) (~100+ models). See [References — Robotics assets](references.qmd#robotics-assets).

| Pattern | When |
|---|---|
| `.gitignore` only | Generated outputs, local envs |
| Git LFS | Small assets that *must* version with *your* code |
| Sparse vendor clone | Third-party model packs you pin but do not fork |
| External URL / HF | Huge datasets, checkpoints |

Attribution: keep upstream `LICENSE` files in the vendor tree; link to Menagerie in docs.
