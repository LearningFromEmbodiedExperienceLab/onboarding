...
This repo’s pattern (`third_party/menagerie.lock.json` + `scripts/fetch_menagerie_assets.sh`):
...
`git sparse-checkout set agilex_piper arx_l5`, then `git checkout <pinned-sha>`.
Everyone gets the same robots without cloning all of Menagerie (~100+ models).
