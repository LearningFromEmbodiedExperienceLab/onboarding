...
A type hint is a note on a name that says what it is supposed to be. Python **does not enforce** this at runtime ([PEP 484](https://peps.python.org/pep-0484/) — passing a `str` where you annotated `float` still runs). The editor, and optional checkers like [Pyright](https://microsoft.github.io/pyright/), read the notes.
...
The language server (Pylance in VS Code, Cursor's Pyright) is a second program. It does **not** use your shell's [`PYTHONPATH`](environment-variables.qmd#pythonpath). It uses:

1. The **selected interpreter** (status bar → `.venv/bin/python`). That is how it finds `numpy` and an editable `robotics`.
2. **`extraPaths`** — extra folders to search, the editor's analogue of [`PYTHONPATH`](environment-variables.qmd#pythonpath).
