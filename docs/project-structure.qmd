...
**2. [`PYTHONPATH`](environment-management.qmd#environment-variables) (temporary; do not rely on this in a team)**

```bash
PYTHONPATH=src python scripts/train.py
```

Puts `src/` on `sys.path`. Convenient for an afternoon, fragile: everyone must remember the same [environment variable](environment-management.qmd#environment-variables), and IDEs / debuggers will not.
