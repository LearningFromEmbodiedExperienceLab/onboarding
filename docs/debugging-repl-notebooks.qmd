...
**ipdb** is pdb with IPython tab-completion and nicer tracebacks. [`PYTHONBREAKPOINT`](environment-variables.qmd#pythonbreakpoint) selects which debugger `breakpoint()` starts:
...
[`PYTHONBREAKPOINT=0`](environment-variables.qmd#pythonbreakpoint) disables every `breakpoint()` for one run (handy when you left one in by mistake).
...
| `env` | Environment variables for **this** debug session ([Common environment variables](environment-variables.qmd#common-environment-variables)). The **print_env** config sets `CUDA_VISIBLE_DEVICES` and `PYTHONPATH` |
