...
ideas that cross MuJoCo, Motrix, URDF, and most other engines.
...
Handy tools: Blender, MeshLab, and Python **`trimesh`** for quick inspection
...
see [Asset files](asset-files.qmd#menagerie-vendor-assets) and
...
(capsules, mesh convex hulls, or boxes for contact). Offscreen renders with only one
group enabled:

![AgileX Piper: visual geoms (left) vs collision geoms (right)](imgs/menagerie/piper_compare.png)

![ARX L5: visual geoms (left) vs collision geoms (right)](imgs/menagerie/arx_l5_compare.png)

Piper collision uses **capsules** on many links but **mesh convex hulls** on others —
the collision side looks smoother and chunkier than the painted mesh. L5 shows **box**
collision primitives on the gripper fingers while the visuals stay detailed meshes — a
typical “art vs physics” split.

Regenerate these figures after bumping `third_party/menagerie.lock.json`:
